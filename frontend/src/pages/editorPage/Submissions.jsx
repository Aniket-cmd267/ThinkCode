...
// import { ArrowLeft, Code2, CheckCircle2, XCircle, Terminal, AlertTriangle } from "lucide-react";
...
//     if (s.includes("accepted")) return "text-emerald-400 bg-emerald-500/10 border-emerald-500/30";
//     if (s.includes("wrong")) return "text-red-400 bg-red-500/10 border-red-500/30";
...
//             Execution Logs
//           </h2>
//           <p className="text-xs text-slate-400 font-medium">Historical Run Analytics</p>
//         </div>
//         <span className="text-xs font-mono font-bold uppercase tracking-wider bg-slate-800 px-3 py-1 rounded-lg text-slate-300">
//           {rows.length} Total Logs
//         </span>
//       </div>

//       {selected ? (
//         <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-200">
//           {/* HEADER BACK-MENU BAR */}
//           <div className="flex flex-wrap items-center justify-between gap-4 bg-[#1F2633] p-4 border border-slate-700/40 rounded-2xl shadow-xl">
//             <button
//               type="button"
//               onClick={() => setSelected(null)}
//               className="inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-xl border border-slate-700 bg-[#161B26] text-slate-300 hover:text-white hover:border-slate-500 transition-all active:scale-95"
//             >
//               <ArrowLeft size={15} /> Back to history
//             </button>

//             <div className="flex flex-wrap items-center gap-2.5 text-xs font-mono">
//               <span className={`px-3 py-1 rounded-lg border font-bold uppercase tracking-wide ${statusStyle(selected?.status)}`}>
//                 {formatStatus(selected?.status)}
//               </span>
//               <span className="px-3 py-1 rounded-lg border border-slate-700/60 bg-[#161B26] text-slate-300 font-semibold uppercase">
//                 {selected?.language || "--"}
//               </span>
//               <span className="px-3 py-1 rounded-lg border border-slate-700/60 bg-[#161B26] text-slate-300">
//                 Time: {formatRuntime(selected?.runtime)}
//               </span>
//               <span className="px-3 py-1 rounded-lg border border-slate-700/60 bg-[#161B26] text-slate-300">
//                 Mem: {formatMemory(selected?.memory)}
//               </span>
//               <span className="px-3 py-1 rounded-lg border border-slate-700/60 bg-[#161B26] text-slate-300">
...
//           {/* TWO COLUMN SIDE-BY-SIDE GRID WORKSPACE */}
//           <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            
//             {/* LEFT COLUMN: CODE VIEWER */}
//             <div className="lg:col-span-7 xl:col-span-8">
//               <CodeBlock
//                 label={`Submitted Template Logic - ${selected?.language || "Unknown"}`}
//                 code={selected?.code || ""}
//                 className="shadow-2xl border border-slate-700/40 rounded-2xl overflow-hidden"
//               />
//             </div>

//             {/* RIGHT COLUMN: RECONCILED MISMATCH DIAGNOSTIC BOX */}
//             <div className="lg:col-span-5 xl:col-span-4 bg-[#1F2633] border border-slate-700/40 rounded-2xl p-5 shadow-2xl space-y-4">
//               <div className="flex items-center justify-between border-b border-slate-700/40 pb-3">
//                 <h4 className="text-xs font-bold text-white uppercase tracking-wider">Evaluation Profile</h4>
//                 <span className="text-xs font-mono font-bold text-slate-400 bg-[#161B26] px-2 py-0.5 rounded border border-slate-700/40">
//                   {selected?.testCasesPassed ?? 0} / {selected?.testCasesTotal ?? 0}
//                 </span>
//               </div>

//               {String(selected?.status).toLowerCase() === "accepted" ? (
//                 /* ALL PASSED BANNER OPTION */
//                 <div className="p-6 bg-emerald-500/5 border border-emerald-500/20 rounded-xl text-center space-y-2">
//                   <div className="flex justify-center text-emerald-400"><CheckCircle2 size={32} /></div>
//                   <p className="text-xs uppercase font-mono tracking-widest text-emerald-400 font-bold">Passed</p>
//                   <h1 className="text-4xl font-black font-mono text-white tracking-tight">
//                     {selected?.testCasesPassed} / {selected?.testCasesTotal}
//                   </h1>
//                   <p className="text-[11px] text-slate-400">All automated sub-assertions evaluated matching runtime criteria perfectly.</p>
//                 </div>
//               ) : String(selected?.status).toLowerCase() === "wrong" && selected?.failedTestCaseDetails ? (
//                 /* EXCLUSIVELY SHOW FAILED TEST CASE PARAMETERS HERE */
//                 <div className="space-y-3 font-mono text-xs">
//                   <div className="flex items-center gap-1.5 font-sans text-xs font-bold text-red-400 uppercase tracking-wide">
//                     <Terminal size={14} /> Mismatch Node Breakdown
//                   </div>
                  
//                   <div className="bg-[#161B26] border border-slate-700/60 p-4 rounded-xl space-y-3.5 shadow-inner">
//                     <div>
//                       <span className="text-[10px] font-sans font-bold uppercase tracking-wider text-slate-500 block mb-1">Input Stream Payload</span>
//                       <div className="bg-[#0B0F19] border border-slate-800 text-slate-200 p-3 rounded-lg overflow-x-auto whitespace-pre-wrap max-h-24 break-all">
...
//                         <div className="bg-[#0B0F19] border border-emerald-950/40 text-emerald-400 p-3 rounded-lg overflow-x-auto font-bold break-all">
...
//                         <div className="bg-[#0B0F19] border border-red-950/40 text-red-400 p-3 rounded-lg overflow-x-auto font-bold break-all">
...
//                 </div>
//               ) : (
//                 /* COMPILATION OR RUNTIME DIAGNOSTIC EXCEPTION ERRORS */
//                 (selected?.errorMessage || selected?.compile_output) && (
//                   <div className="space-y-2 font-mono text-xs">
//                     <div className="flex items-center gap-1.5 font-sans text-xs font-bold text-red-400 uppercase tracking-wide">
//                       <AlertTriangle size={14} /> Diagnostic Error Logs
//                     </div>
//                     <pre className="w-full bg-[#161B26] border border-red-500/20 p-4 rounded-xl text-red-300 text-[11px] overflow-x-auto whitespace-pre-wrap max-h-48 leading-relaxed shadow-inner">
//                       {selected?.errorMessage || selected?.compile_output}
//                     </pre>
//                   </div>
//                 )
...
//         </div>
//       ) : rows.length === 0 ? (
//         <div className="rounded-2xl border border-dashed border-slate-800 bg-[#1F2633]/30 p-10 text-center text-slate-500 font-mono text-xs uppercase tracking-wide">
//           No metrics submitted yet. Execute challenges to compile records.
//         </div>
//       ) : (
//         /* CORE LOG LIST WORKSPACE */
//         <div className="rounded-2xl border border-slate-800 bg-[#1F2633] overflow-hidden shadow-2xl">
//           <div className="grid grid-cols-12 gap-4 px-5 py-3.5 bg-[#161B26] border-b border-slate-800 text-[10px] font-bold uppercase tracking-wider text-slate-400 font-mono">
//             <div className="col-span-1">#</div>
//             <div className="col-span-3 lg:col-span-2">Execution State</div>
//             <div className="col-span-2">Language</div>
//             <div className="col-span-2">Runtime</div>
//             <div className="col-span-2">Memory</div>
//             <div className="col-span-1">Assertions</div>
//             <div className="col-span-1 hidden lg:block">Timestamp</div>
...
//                 className="grid grid-cols-12 gap-4 px-5 py-3.5 items-center hover:bg-[#161B26]/40 transition-colors"
//               >
//                 <div className="col-span-1 text-slate-500 font-mono text-xs font-bold">{index + 1}</div>
//                 <div className="col-span-3 lg:col-span-2">
//                   <span className={`inline-flex px-2.5 py-0.5 rounded-md border text-xs font-semibold uppercase font-mono tracking-wide ${statusStyle(submission?.status)}`}>
//                     {formatStatus(submission?.status)}
//                   </span>
//                 </div>
//                 <div className="col-span-2 text-slate-300 font-mono text-xs uppercase font-semibold">{submission?.language || "--"}</div>
...
//                 </div>
//                 <div className="col-span-1 text-slate-300 text-xs font-mono font-bold">
//                   {submission?.testCasesPassed ?? 0} <span className="text-slate-500">/</span> {submission?.testCasesTotal ?? 0}
//                 </div>
//                 <div className="col-span-1 text-slate-400 text-xs hidden lg:block font-medium">
...
//                     className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl border border-slate-700 bg-[#161B26] text-slate-300 hover:text-white hover:border-slate-500 transition-all active:scale-95"
//                   >
//                     <Code2 size={14} />
//                     View
//                   </button>
      ...
      {selected ? (
        <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {/* RIGHT COLUMN CONTAINER: LEETCODE ANALYTICAL DIAGNOSTIC WORKSPACE */}
            <div className="lg:col-span-5 xl:col-span-4 space-y-5">
              
              {/* PRIMARY DIAGNOSTIC PROFILE LAYER */}
              <div className="bg-[#121824] border border-slate-800/80 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-5">
                  {/* <span className="text-xs font-bold text-slate-400 uppercase tracking-wider font-mono">Report Diagnostics</span> */}
